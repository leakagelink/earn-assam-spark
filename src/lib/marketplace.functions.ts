...
export const runMemberAction = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (data.action === "notification" && data.notificationId) {
      ...
      const { error } = await context.supabase.from("emergency_contacts").upsert({ id: data.contactId, user_id: context.userId, name: data.text, phone: data.secondary, relation: "Emergency", is_primary: true }); if (error) throw error;
...
export const runAdminAction = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => adminActionSchema.parse(input)).handler(async ({ context, data }) => {
  ...
  else if (data.action === "commission" && data.id) ({ error } = await supabaseAdmin.from("commission_settings").update({ commission_percent: data.value, minimum_withdrawal: data.secondary }).eq("id", data.id));
  ...
  else if (data.action === "report" && data.id) ({ error } = await supabaseAdmin.from("user_reports").update({ status: data.status, details: data.text || null }).eq("id", data.id));
  else if (data.action === "dispute" && data.id) ({ error } = await supabaseAdmin.from("booking_disputes").update({ status: data.status, resolution: data.text || null, reviewed_by: context.userId }).eq("id", data.id));
  else if (data.action === "refund" && data.id) { ({ error } = await supabaseAdmin.from("refunds").update({ status: data.status, admin_note: data.text || null, reviewed_by: context.userId, reviewed_at: new Date().toISOString() }).eq("id", data.id)); if (data.status === "processed") { const { data: refund } = await supabaseAdmin.from("refunds").select("payment_id,booking_id").eq("id", data.id).single(); if (refund) { await supabaseAdmin.from("payments").update({ status: "refunded" }).eq("id", refund.payment_id); await supabaseAdmin.from("bookings").update({ payment_status: "refunded" }).eq("id", refund.booking_id); } } }
