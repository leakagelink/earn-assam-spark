...
function AccountPage() {
  ...
  return <AppShell title="My account" eyebrow="Customer workspace">
    ...
    <section className="grid gap-5 lg:grid-cols-[1.4fr_1fr]">
      ...
      <div className="glass-panel rounded-xl p-5 sm:p-7"><p className="text-xs uppercase text-primary">Quick actions</p><div className="mt-4 grid gap-3"><Button asChild variant="light"><Link to="/provider/setup"><BriefcaseBusiness /> {workspace?.provider ? "Edit provider profile" : "Become a provider"}</Link></Button><Button asChild variant="glass"><Link to="/dashboard"><CalendarDays /> Bookings & earnings</Link></Button>{workspace?.roles.includes("admin") && <Button asChild variant="kinetic"><Link to="/admin"><ShieldCheck /> Admin control</Link></Button>}</div></div>
    </section>
